...
int eio_map(struct cache_c *dmc, struct request_queue *rq, struct bio *bio)
{
	...
	pr_debug("new I/O, dir=%d/%d, idx=%u, sector=%llu, size=%u, vcnt=%d, current_prio=%u",
	         data_dir, bio_op(bio), EIO_BIO_BI_IDX(bio), (long long unsigned int) EIO_BIO_BI_SECTOR(bio), EIO_BIO_BI_SIZE(bio), bio->bi_vcnt, get_current_ioprio());
	...
	if (EIO_BIO_BI_IDX(bio) != 0 || bio_flagged(bio, BIO_CHAIN)) {
		...
		pr_debug("eio_map: pass-trought non zero idx or chained, dir=%d/%d, idx=%u, sector=%llu, size=%u, vcnt=%d\n",
			data_dir, bio_op(bio), EIO_BIO_BI_IDX(bio), (long long unsigned int) EIO_BIO_BI_SECTOR(bio), EIO_BIO_BI_SIZE(bio), bio->bi_vcnt);
